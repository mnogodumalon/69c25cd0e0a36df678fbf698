...
interface MitgliederverwaltungViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: Mitgliederverwaltung | null;
  onEdit: (record: Mitgliederverwaltung) => void;
}

export function MitgliederverwaltungViewDialog({ open, onClose, record, onEdit }: MitgliederverwaltungViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Mitgliederverwaltung anzeigen</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Jahresbeitrag bezahlt am</Label>
            <p className="text-sm">{formatDate(record.fields.jahresbeitrag_status)}</p>
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Abteilung im Verein</Label>
