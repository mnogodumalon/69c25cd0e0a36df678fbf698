...
export function useDashboardData() {
  ...
  const fetchAll = useCallback(async () => {
    setError(null);
    try {
      const [vereinsversammlungenData, mitgliederverwaltungData] = await Promise.all([
  ...
  return { vereinsversammlungen, setVereinsversammlungen, mitgliederverwaltung, setMitgliederverwaltung, loading, error, fetchAll, mitgliederverwaltungMap };
}